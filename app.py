...
st.set_page_config(
    page_title="Medivelius - Paid Media Dashboard",
    page_icon="🧴",
    layout="wide"
)
...
with st.sidebar:
    try:
        st.image("logo_medivelius_dashboard.png", use_container_width=True)
    except:
        st.caption("🧴 *Subir 'logo_medivelius_dashboard.png' a GitHub para activar el logo personalizado*")
        
    st.markdown("## 📊 Control de Paid Media")
    st.write("Propiedad: **Medivelius**")
    st.markdown("---")
    mes_seleccionado = st.selectbox("📅 Seleccione el Mes de Reporte:", options=meses_disponibles)

# --- CONEXIÓN DINÁMICA CON LA URL PROPIA Y VERIFICADA DE MEDIVELIUS ---
url_base = "https://docs.google.com/spreadsheets/d/1K7_8Iy-l8f9yWvE9C8f6sIunO_b10YfA8LpX9w-N2_g/"
...
try:
    ...
    # Fila de inicio estándar para la matriz
    idx_header = 2 
    ...
    # 2. CAPTURA DE MATRIZ DE DATOS REALES DE MEDIVELIUS
    df_datos = df_raw.iloc[idx_header + 1:].copy()
    
    # Asignación de índices de columnas fijos y validados
    col_idx_medio = 0  # Columna A: Channel
    col_idx_camp = 1   # Columna B: Campaign
    col_idx_status = 4 # Columna E: Status (Activa / Pausada) -> Mapeado con Éxito
    ...
    # --- INTERFAZ VISUAL ---
    st.title(f"🧴 Dashboard Gerencial Medivelius: {mes_seleccionado.title()}")
...
st.caption(f"Medivelius Dermocosmetica Analytics | Strategic Analytics by goBIG")
